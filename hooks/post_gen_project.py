...
MAKOO_SITESCONS = 'https://github.com/eyob-boye/makoo_site_scons'
...
def setup_git_repo():
    ...
    run(['git', 'status'])
    run(['git', 'add', '-A'])
    run(['git', 'submodule', 'add', MAKOO_SITESCONS, '{{cookiecutter.package_name}}/site_scons'])
    run(['git', 'status'])
    run(['git', 'commit', '-m', 'Initial commit'])
    #run('scons', '--directory={{cookiecutter.repository_slug}}\{{cookiecutter.package_name}}')
    #run(['git', 'add', '-A'])
    #run(['git', 'commit', '-m', 'Added auto generated kicad project files'])
